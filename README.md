...
- [容器部署指南](docs/deployment.md) —— rootless / rootful 自动判定、一键部署、HTTPS / RustFS 反代、证书
- [自行部署指南](docs/self-hosting.md) —— `./deploy.sh self` 打包产物 + 步骤，运行方式由你自行决定
- [GitHub Actions 自动部署](docs/ci.md) —— CI 下发流程与 `remote-deploy.sh` 说明
- [本地开发环境](docs/development.md) —— 后端 `go run`、前端 `pnpm dev`
