...
// The PostCategoryRelationFunc type is an adapter to allow the use of ordinary
// function as PostCategoryRelation mutator.
type PostCategoryRelationFunc func(context.Context, *ent.PostCategoryRelationMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f PostCategoryRelationFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.PostCategoryRelationMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.PostCategoryRelationMutation", m)
}
